...
def make_plot(probs, chosen_id, step, method, top_n=15):
    top_probs, top_ids = torch.topk(probs, top_n)
    top_ids = top_ids.tolist()
    top_probs = top_probs.tolist()

    # The sampled token can fall outside the displayed top-N (this happens
    # routinely with top-p, whose nucleus is often much wider than top_n).
    # If so, swap it in for the lowest-shown entry so what was actually
    # picked is always visible, then re-sort by probability.
    if chosen_id not in top_ids:
        top_ids[-1] = chosen_id
        top_probs[-1] = probs[chosen_id].item()
        pairs = sorted(zip(top_ids, top_probs), key=lambda x: -x[1])
        top_ids, top_probs = zip(*pairs)

    tokens = [tokenizer.decode(i).strip() or "·" for i in top_ids]
    values = np.array(top_probs) * 100
...
def run_generation(prompt, method, temperature, top_k, top_p, max_new_tokens, seed, delay):
...
with gr.Blocks(title="Decoding Strategy Playground") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            delay = gr.Slider(0.0, 2.0, value=0.4, step=0.05,
                               label="Delay between tokens (seconds) — controls streaming speed")
    ...
    generate_btn.click(
        run_generation,
        inputs=[prompt, method, temperature, top_k, top_p, max_new_tokens, seed, delay],
